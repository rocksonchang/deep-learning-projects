...
'''
'''
Minor update to explicitly split autoencoder into encoder and decoder
RC, Nov 5, 2017
'''
...
build_model(shape=original_img_size, n_filters=n_filters, kernal_size=kernal_size, use_transposed_conv=use_transposed_conv)
    ...
    ############## End of encoder ##############
    # The compressed representation is (4, 4, 32) 
    ...
    # encoder    
    n_encoder_layers = 22
    enco = autoencoder.layers[](x)
    for i in range(1,n_encoder_layers):
        enco = autoencoder.layers[i](deco)        
...
def train(job_dir=None, job_id=None,
          ...
          patience=5, batch_norm_before_activation=False, **kwargs):
    ...
    ########## Build models ##############
    autoencoder, _, _ = build_model(shape=original_img_size, 
                                                n_filters=n_filters,
