...
import data_sources              # mnist and fashion-mnist
...
def build_model(shape, n_filters, kernel_size, use_transposed_conv):
    ############ Encoder ###############
    x = Input(shape=shape) # reshape to: (100, 28, 28, 1) 
    ...
    # End of encoder. The compressed representation is (4, 4, 8*4) 
...
def build_model_2(shape, n_filters, kernel_size, use_transposed_conv):
    ############ Encoder ###############
    x = Input(shape=shape) # reshape to: (100, 28, 28, 1) 
...
def train(job_dir=None, job_id=None,
          ...
          patience=5, batch_norm_before_activation=False, dataset='mnist', **kwargs):
    ...
    n_filters = 8*2 # number of convolutional filters to use
    ...
    if job_id is None:
        ...
        if 'gs://' in job_dir:
            ...
            checkpoint_filename = job_dir + job_id + '_autoencoder_checkpoint_{}.hdf5'.format(job_id)
    ...
    autoencoder = build_model(shape=original_img_size, 
                              n_filters=n_filters, 
    ...
    if dataset == 'mnist':
      ...
      (x_train, _), (x_test, y_test) = data_sources.load_mnist()
    elif dataset == 'mnistf':
      # data from fashion MNIST. 
      if 'gs://' in job_dir:        
        BUCKET_NAME = 'rc_bucket'
        data_path = 'gs://{}/data/fashion_mnist'.format(BUCKET_NAME)
        (x_train, _), (x_test, y_test) = data_sources.load_fmnist(path=data_path)
      else:
    ...
    ######### Evaluate trained model on test set Part 1 ############    
    ...
    ######### Evaluate trained model on test set Part 2 ############    
    ...
    for i in range(n_imgs_to_show):
        axes[i//n_cols * 2, i % n_cols].imshow(x_test_sub[i,:,:,0], cmap='Greys')
        axes[i//n_cols * 2, 0].set_ylabel('Original')
        axes[i//n_cols * 2 +1, i % n_cols].imshow(reconstructed_test[i,:,:,0], cmap='Greys')
